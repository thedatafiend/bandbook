...
import type { BandMember, UserBand } from "@/lib/queries";
...
export function SettingsView({
  ...
  // Band name / invite token are editable in place — track them locally,
  // seeded from the server-rendered props.
    ...
    <main className="flex flex-1 flex-col px-6 py-8 max-w-lg mx-auto w-full">
      ...
      <div className="flex flex-col gap-8">
        <BandNameSection
          ...
          onUpdate={(newName) => setBand((b) => ({ ...b, name: newName }))}
        />
        <InviteLinkSection
          inviteToken={band.invite_token}
          onRegenerate={(newToken) =>
            setBand((b) => ({ ...b, invite_token: newToken }))
          }
        />
...
function MembersSection({ members }: { members: BandMember[] }) {
